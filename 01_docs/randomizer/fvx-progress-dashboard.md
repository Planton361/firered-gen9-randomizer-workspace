...
| Zuletzt entblockt | UPR-FVX PR #116 finalisiert `MODE-TRAINER-CLASS-SPRITE-SYNC`: `Randomize Trainer Names` bleibt personal-name-only; `Randomize Trainer Class Names` bleibt ohne Sync legacy/textlabel-only; mit Sync folgen Class label, `trainerClass` und sichtbarer `trainerPic` der Class-Assignment; sanitized local smoke bestaetigt per-trainer Regular assignments, gruppierte Rival/Friend class/sprite consistency, eligible Runtime-Source-Sync und keine gemeldeten garbled sprites/crashes; PR #109 fixt die CFRU/DPE Gen9 BPRE Intro-Mon Visual-Source fuer den bestaetigten Nidoran-female Asset-Table-Scope; PR #106 stellt das Post-Audit-Tooling bereit; PR #105 macht generische `RUNTIME-SOURCE` Trainer randomizer-eligible; sanitized local evidence bestaetigt Viridian Forest `531/532` mit randomized loaded/raw parties, Ingame-Smoke Eiscue und randomized-output Audit `unloaded-valid-parties total=0` |
| Aktuelle Caveats | Trainer Class Sprite Sync ist targeted visual-smoke-bestaetigt, aber kein Full-Playthrough, keine globale Route-/Kategorie-Abdeckung und keine P1-Promotion; Intro-Mon Visual-Mismatch ist targeted ingame-smoke-bestaetigt gefixt, aber kein Full-Playthrough und keine P1-Promotion; Runtime-source strict sync plus randomizer eligibility ist fuer Viridian Forest `531/532` lokal bestaetigt, aber keine breite Trainer/Foe-P1-Promotion; `loaded-mismatch`, invalid-pointer, empty-party und out-of-range rows sowie Full-Playthrough bleiben Diagnose-/Follow-up-Scope; Graphics/Palettes und Misc Tweaks brauchen Visual-/Behavior-Smokes; Gen-Limit-1-9 `MODE-*` Overlays bleiben unsupported by Settings format; Special-Wild/Day-Night/Swarms bleiben separater Scope; `Rival Carries Starter Through Game` hat CLI-Log-Smoke, aber keinen Ingame-/Full-Rival-Smoke |
| Keine P1-Promotion | Aktuelle Updates sind Workspace-/Smoke-Status, keine neue P1-Freigabe |
| Naechster sinnvoller Block | Trainer-Class-Sprite-Sync nur noch bei Regression oder breiterer Sampling-Frage weiter vertiefen; danach `loaded-mismatch`/`invalid` Runtime-Source-Rows separat triagieren; weitere suspected runtime-source battles nur mit eigener sanitized Evidence aufnehmen; gezielt Ingame-/Visual-/Behavior-Smokes isolieren: Trainer Held Items Sensible, Graphics/Palettes, Misc Tweaks, Special-Wild und Full-Rival-Carry |
...
| Foe Pokemon / Trainer | Working-matrix passed mit Caveat / exact coverage passed / Runtime-source 531-532 smoke passed / Trainer Class Sprite Sync targeted-smoke passed | Trainer Pokemon core, Trainer Movesets, Trainer Names; `FVX-FOE-001` bis `014` and exact Foe mode overlays CLI log-smoked; `MODE-TRAINER-CLASS-SPRITE-SYNC` targeted-smoke-confirmed for class label/classId/pic consistency; strict `VALID_RUNTIME_NOT_LOADED` runtime-source sync merged; generic `RUNTIME-SOURCE` trainers randomizer-eligible; Viridian Forest `531/532` locally confirmed; randomized output audit `unloaded-valid-parties total=0`; Rival 2 `329/330/331` and Brock `414` randomized in sanitized observations | Additional Pokemon, Type Diversity / Type Themes, Battle Style, Full-Rival-Carry, Trainer Held Items, Trainer Class Sprite Sync and exact Foe modes log-smoked or documented | Runtime-source `531/532` Trainer Pokemon path is locally smoke-confirmed, but broader loaded-mismatch/invalid/out-of-range rows and full playthrough remain Follow-up; Trainer Class Sprite Sync is targeted visual-smoke-confirmed only; without Sync Trainer Class Names remains legacy/textlabel-only; Sensible Held Items braucht fokussierte Isolation; Full-Rival-Carry braucht Ingame-Smoke | `loaded-mismatch`/`invalid` separat triagieren; weitere suspected runtime-source battles nur mit eigener sanitized Evidence aufnehmen; Sensible Items, Graphics/Palettes, Misc Tweaks und Full-Rival-Carry isolieren; Trainer-Class-Sprite-Sync nur bei Regression/breiterem Sampling erneut vertiefen | 190, 191, 197, 198, 199, 201, 202, 203, 204, 206, PR116 |
...
| 2 | Stable Visual Profile + Starter Pokemon + Intro Mon | groesstes aktuelles Normalprofil laenger samplen | Stable Visual Profile passed, Oak-Lab Rival Sync passed und Intro Mon visual fix targeted-smoke passed separat | lokaler Smoke mit Starters ON, Intro Mon ON, Trainer Class Names OFF, Special-Wild OFF |
