...
|---|---|
| Stand | Nach UPR-FVX PR #124 Expanded-Trainer-Logging-Fallback, PR #123 Gen3/CFRU-DPE Palette-Output-Writes, Graphics/Palettes Visual Smoke 209, PR #118 Wild Encounter Base-vs-Output Audit, Combined Trainer Visual Runtime Smoke 208, UPR-FVX PR #117 Rival-Counter-Starter/Intro-Species-0-Fix und combined visual smoke, PR #116 finalem Trainer Class Sprite Sync Smoke, PR #109 Intro-Mon Visual-Source-Fix-Smoke, PR #107/#108 Intro-Mon Visual-Source-Diagnose, PR #106 Pre/Post Runtime-Trainer-Audit, PR #105 Runtime-Source-Trainer-Randomization-Smoke, PR #104 Strict-Runtime-Trainer-Source-Sync, coverage-generated CLI profile matrix run und exact-coverage Batches 01 bis 18 CLI-Log-Smokes |
| UPR-FVX-Pin im Workspace | `0eb815418470fa1ac000695b95d09cb084338dca` |
...
| Zuletzt entblockt | Graphics/Palettes Visual Smoke 209 bestaetigt targeted visual/audit pass mit Caveats: `Pokemon Palettes: Randomized/Changed`, CFRU-DPE palette copy save `normalPaletteWriteAttempts=841`, Palette Audit `sampledCount=21`, `normalChangedCount=21`, `shinyChangedCount=0`, `unchangedCount=0`, sampled normal palettes fuer Charmander/Squirtle/Caterpie/Pikachu/Blissey changed from base, sichtbare geaenderte Paletten und kein finales `Error during logging`; UPR-FVX PR #123 schreibt Gen3/CFRU-DPE Palette-Randomization-Output, PR #124 verhindert den Expanded-Trainer-Logging-Crash; UPR-FVX PR #118 stellt den opt-in Wild Encounter Base-vs-Output Audit bereit: diagnostic-only, keine Writer-/Randomizer-Verhaltensaenderung, modeled Gen3 base `WildPokemon` table path mit Slot-Vergleich und Summary fuer total/changed/unchanged/changed percentage; Combined Trainer Visual Runtime Smoke 208 bestaetigt `PASS_WITH_CAVEATS`: Intro Mon sichtbar randomisiert, Player starter Charmander, Oak-Lab Rival starter Squirtle, Route-22 Rival starter Squirtle, Route-22-Rival-Sprite randomisiert und konsistent mit Oak-Lab-Rival-Sprite, Viridian-Forest-Trainer-Sprites randomisiert und keine crashes/freezes/garbled sprites beobachtet; Route-22 Rival non-starter Pokemon Silvally Lv9 zeigt, dass `Rival Carries Starter Through Game` den Rival-Starter-Slot schuetzt/korrigiert, waehrend Non-Starter-Rival-Pokemon fuer Foe-Pokemon-Randomization eligible bleiben; UPR-FVX PR #117 fixt `Rival Carries Starter Through Game` nach Foe Pokemon Randomization und verhindert Intro-Mon-Species-0-Writes im extended CFRU/DPE BPRE-Pool; PR #116 finalisiert `MODE-TRAINER-CLASS-SPRITE-SYNC`: `Randomize Trainer Names` bleibt personal-name-only; `Randomize Trainer Class Names` bleibt ohne Sync legacy/textlabel-only; mit Sync folgen Class label, `trainerClass` und sichtbarer `trainerPic` der Class-Assignment; sanitized local smoke bestaetigt per-trainer Regular assignments, gruppierte Rival/Friend class/sprite consistency, eligible Runtime-Source-Sync und keine gemeldeten garbled sprites/crashes; PR #109 fixt die CFRU/DPE Gen9 BPRE Intro-Mon Visual-Source fuer den bestaetigten Nidoran-female Asset-Table-Scope |
| Aktuelle Caveats | Graphics/Palettes 209 ist targeted visual/audit smoke, kein Full-Playthrough, keine breite species/form coverage, keine Shiny-Proof-Abdeckung (`shinyChangedCount=0`) und keine P1-Promotion; Wild Encounter Base-vs-Output Audit ist diagnostic-only und deckt den modeled Gen3 base `WildPokemon` table path ab; CFRU/DPE special/runtime wild sources bleiben Follow-up, falls Audit und Ingame-Beobachtung auseinanderlaufen; Combined Trainer Visual Runtime Smoke 208 bleibt `PASS_WITH_CAVEATS`, weil es targeted visual/runtime smoke ist: kein Full-Playthrough, keine all-starter-choice matrix und keine P1-Promotion; Trainer Class Sprite Sync ist targeted visual-smoke-bestaetigt, aber kein Full-Playthrough, keine globale Route-/Kategorie-Abdeckung und keine P1-Promotion; Intro-Mon Species-0-Regressionsfix ist targeted smoke-bestaetigt, aber keine globale Intro-Mon-Speziesabdeckung; Runtime-source strict sync plus randomizer eligibility ist fuer Viridian Forest `531/532` lokal bestaetigt, aber keine breite Trainer/Foe-P1-Promotion; `loaded-mismatch`, invalid-pointer, empty-party und out-of-range rows sowie Full-Playthrough bleiben Diagnose-/Follow-up-Scope; Misc Tweaks brauchen Behavior-Smokes; Gen-Limit-1-9 `MODE-*` Overlays bleiben unsupported by Settings format; Special-Wild/Day-Night/Swarms bleiben separater Scope |
