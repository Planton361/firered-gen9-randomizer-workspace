...
|---|---|
| Stand | Nach UPR-FVX PR #104 Strict-Runtime-Trainer-Source-Sync, PR #103 Runtime-Trainer-Source-Audit-Sync, coverage-generated CLI profile matrix run und exact-coverage Batches 01 bis 18 CLI-Log-Smokes |
| UPR-FVX-Pin im Workspace | `6dcda7e499cd3e22319c447c7d7df9ddbd67de60` |
...
| Zuletzt entblockt | UPR-FVX PR #104 synchronisiert strict valide `trainerbattle` runtime-source TrainerData rows aus der Audit-Klasse `VALID_RUNTIME_NOT_LOADED`; Viridian Forest `531/532` sollten dadurch abgedeckt sein, wenn sie lokal weiterhin so klassifiziert werden; PR #103 stellt den opt-in globalen Runtime-Trainer-Source-Audit bereit; exact-coverage Batches 03-18 filled generator-capable CLI log-smoke coverage across TM/Tutor, Wild, Foe, General/Traits, Starters/Statics/Trades, Moves, Graphics/Palettes, Misc, Types, cumulative profiles and exact Foe/Wild/Type/Intro mode overlays |
| Aktuelle Caveats | Strict Runtime Trainer Source Sync ist gepinnt, aber kein Ersatz fuer private-ROM Audit oder Ingame-Smoke; Foe Trainer bleibt CLI-log-clean, nicht P1-promoted; `loaded-mismatch`, invalid-pointer, empty-party und out-of-range rows bleiben Diagnose-/Follow-up-Scope; Trainer Class Names bleibt textlabel-only; Graphics/Palettes und Misc Tweaks brauchen Visual-/Behavior-Smokes; Intro Mon braucht visuelle Bestaetigung; Gen-Limit-1-9 `MODE-*` Overlays bleiben unsupported by Settings format; Special-Wild/Day-Night/Swarms bleiben separater Scope; `Rival Carries Starter Through Game` hat CLI-Log-Smoke, aber keinen Ingame-/Full-Rival-Smoke |
| Keine P1-Promotion | Aktuelle Updates sind Workspace-/Smoke-Status, keine neue P1-Freigabe |
| Naechster sinnvoller Block | Lokaler private-ROM Audit plus Ingame-Smoke fuer strict synced runtime trainer rows, einschliesslich Viridian Forest `531/532` falls weiterhin `VALID_RUNTIME_NOT_LOADED`; `loaded-mismatch`/`invalid` separat auswerten; danach gezielt Ingame-/Visual-/Behavior-Smokes isolieren: Trainer Held Items Sensible, Graphics/Palettes, Misc Tweaks, Special-Wild und Full-Rival-Carry |
...
| Foe Pokemon / Trainer | Working-matrix passed mit Caveat / exact coverage passed / Strict runtime-source sync pinned | Trainer Pokemon core, Trainer Movesets, Trainer Names; `FVX-FOE-001` bis `014` and exact Foe mode overlays CLI log-smoked; strict `VALID_RUNTIME_NOT_LOADED` runtime-source sync merged; global runtime-source audit opt-in verfuegbar | Additional Pokemon, Type Diversity / Type Themes, Battle Style, Full-Rival-Carry, Trainer Held Items and exact Foe modes log-smoked | Strict sync braucht lokalen private-ROM Audit plus Ingame-Smoke; Viridian Forest `531/532` sollten abgedeckt sein, wenn weiterhin `VALID_RUNTIME_NOT_LOADED`; `loaded-mismatch`/invalid rows bleiben Follow-up; Trainer Class Names textlabel-only; Sprite/Class-ID mismatch erwartbar; Sensible Held Items braucht fokussierte Isolation; Full-Rival-Carry braucht Ingame-Smoke | Lokaler private-ROM Audit plus Ingame-Smoke fuer strict synced runtime rows; `loaded-mismatch`/`invalid` separat triagieren; Trainer Class Names OFF lassen oder Class Assignment separat planen; Sensible Items und Full-Rival-Carry isolieren | 190, 191, 197, 198, 199, 201, 202 |
...
| Trainer | Working-matrix passed mit Caveat / exact coverage passed / Strict runtime-source sync pinned | Species, Movesets, Trainer Names; strict `VALID_RUNTIME_NOT_LOADED` runtime-source sync merged; global runtime-source audit opt-in verfuegbar | Additional Pokemon, Type Diversity / Type Themes, Special Rules, Battle Style, Full-Rival-Carry, Held Items and exact Foe modes log-smoked | Strict sync braucht lokalen private-ROM Audit plus Ingame-Smoke; Viridian Forest `531/532` sollten abgedeckt sein, wenn weiterhin `VALID_RUNTIME_NOT_LOADED`; `loaded-mismatch`/invalid rows bleiben Follow-up; Trainer Class Names textlabel-only; Sensible Held Items und Full-Rival-Carry brauchen fokussierte Ingame-Smokes | Private-ROM Audit plus Ingame-Smoke fuer strict synced runtime rows; `loaded-mismatch`/`invalid` separat triagieren; Trainer Class Names fuer Stable-Visual OFF lassen; Sensible Items und Full-Rival-Carry isolieren |
