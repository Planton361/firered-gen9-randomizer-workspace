...
| Zuletzt entblockt | UPR-FVX PR #117 fixt `Rival Carries Starter Through Game` nach Foe Pokemon Randomization und verhindert Intro-Mon-Species-0-Writes im extended CFRU/DPE BPRE-Pool; sanitized local smoke bestaetigt combined visual Rival test fixed, sichtbaren Blissey Intro Mon, Player Charmander -> Rival Squirtle und keine gemeldeten crashes/freezes/garbled sprites; PR #116 finalisiert `MODE-TRAINER-CLASS-SPRITE-SYNC`: `Randomize Trainer Names` bleibt personal-name-only; `Randomize Trainer Class Names` bleibt ohne Sync legacy/textlabel-only; mit Sync folgen Class label, `trainerClass` und sichtbarer `trainerPic` der Class-Assignment; sanitized local smoke bestaetigt per-trainer Regular assignments, gruppierte Rival/Friend class/sprite consistency, eligible Runtime-Source-Sync und keine gemeldeten garbled sprites/crashes; PR #109 fixt die CFRU/DPE Gen9 BPRE Intro-Mon Visual-Source fuer den bestaetigten Nidoran-female Asset-Table-Scope |
| Aktuelle Caveats | Rival Carries Starter Through Game und combined visual profile sind targeted-smoke-bestaetigt, aber kein Full-Playthrough, keine all-starter-choice matrix und keine P1-Promotion; Trainer Class Sprite Sync ist targeted visual-smoke-bestaetigt, aber kein Full-Playthrough, keine globale Route-/Kategorie-Abdeckung und keine P1-Promotion; Intro-Mon Species-0-Regressionsfix ist targeted smoke-bestaetigt, aber keine globale Intro-Mon-Speziesabdeckung; Runtime-source strict sync plus randomizer eligibility ist fuer Viridian Forest `531/532` lokal bestaetigt, aber keine breite Trainer/Foe-P1-Promotion; `loaded-mismatch`, invalid-pointer, empty-party und out-of-range rows sowie Full-Playthrough bleiben Diagnose-/Follow-up-Scope; Graphics/Palettes und Misc Tweaks brauchen Visual-/Behavior-Smokes; Gen-Limit-1-9 `MODE-*` Overlays bleiben unsupported by Settings format; Special-Wild/Day-Night/Swarms bleiben separater Scope |
...
| Foe Pokemon / Trainer | Working-matrix passed mit Caveat / exact coverage passed / Runtime-source 531-532 smoke passed / Rival Carry targeted-smoke passed / Trainer Class Sprite Sync targeted-smoke passed | Trainer Pokemon core, Trainer Movesets, Trainer Names; `FVX-FOE-001` bis `014` and exact Foe mode overlays CLI log-smoked; Rival Carries Starter Through Game targeted-smoke-confirmed for Player Charmander -> Rival Squirtle after Foe Pokemon randomization; `MODE-TRAINER-CLASS-SPRITE-SYNC` targeted-smoke-confirmed for class label/classId/pic consistency; strict `VALID_RUNTIME_NOT_LOADED` runtime-source sync merged; generic `RUNTIME-SOURCE` trainers randomizer-eligible; Viridian Forest `531/532` locally confirmed; randomized output audit `unloaded-valid-parties total=0`; Rival 2 `329/330/331` and Brock `414` randomized in sanitized observations | Additional Pokemon, Type Diversity / Type Themes, Battle Style, Full-Rival-Carry, Trainer Held Items, Trainer Class Sprite Sync and exact Foe modes log-smoked or documented | Rival Carry targeted smoke covers one sampled counter path, not all starter choices or full playthrough; Runtime-source `531/532` Trainer Pokemon path is locally smoke-confirmed, but broader loaded-mismatch/invalid/out-of-range rows and full playthrough remain Follow-up; Trainer Class Sprite Sync is targeted visual-smoke-confirmed only; without Sync Trainer Class Names remains legacy/textlabel-only; Sensible Held Items braucht fokussierte Isolation | `loaded-mismatch`/`invalid` separat triagieren; weitere suspected runtime-source battles nur mit eigener sanitized Evidence aufnehmen; Sensible Items, Graphics/Palettes und Misc Tweaks isolieren; Rival Carry und Trainer-Class-Sprite-Sync nur bei Regression/breiterem Sampling erneut vertiefen | 190, 191, 197, 198, 199, 201, 202, 203, 204, 206, 207, PR116, PR117 |
...
| Trainer | Working-matrix passed mit Caveat / exact coverage passed / Runtime-source 531-532 smoke passed / Trainer Class Sprite Sync targeted-smoke passed | Species, Movesets, Trainer Names; `MODE-TRAINER-CLASS-SPRITE-SYNC` fuer class label/classId/pic consistency targeted-smoke-confirmed; strict `VALID_RUNTIME_NOT_LOADED` runtime-source sync merged; generic `RUNTIME-SOURCE` trainers randomizer-eligible; Viridian Forest `531/532` locally confirmed; randomized output audit `unloaded-valid-parties total=0`; Rival 2 `329/330/331` and Brock `414` randomized in sanitized observations | Additional Pokemon, Type Diversity / Type Themes, Special Rules, Battle Style, Full-Rival-Carry, Held Items, Trainer Class Sprite Sync and exact Foe modes log-smoked/documented | Broader `loaded-mismatch`/invalid/out-of-range rows and full playthrough remain Follow-up; Trainer Class Sprite Sync targeted smoke is not full-playthrough proof; without Sync Trainer Class Names remains legacy/textlabel-only; Sensible Held Items und Full-Rival-Carry brauchen fokussierte Ingame-Smokes | `loaded-mismatch`/`invalid` separat triagieren; weitere suspected runtime-source battles nur mit eigener sanitized Evidence aufnehmen; Sensible Items und Full-Rival-Carry isolieren; Trainer-Class-Sprite-Sync nur bei Regression/breiterem Sampling erneut vertiefen |
...
| 3 | `FVX-GEN-003` | General Options | No Random Intro Mon | Intro visual fix targeted-smoke passed; no P1 promotion | Negative GUI option; `MODE-NO-RANDOM-INTRO` / `FVX-GEN-003` set `randomizeIntroMon=false`; PR #109 fixes confirmed CFRU/DPE visual source; full-playthrough still open |
...
| 70 | `FVX-FOE-012` | Foe Pokemon | Rival Carries Starter Through Game | Targeted visual smoke passed | Player Charmander -> Rival Squirtle; no P1 promotion |
| 71 | `FVX-FOE-013` | Foe Pokemon | Randomize Trainer Names / Class Names | Trainer Names passed; Class Names textlabel-only without Sync; Sprite Sync targeted-smoke passed | `MODE-TRAINER-CLASS-SPRITE-SYNC` targeted visual smoke; regular per-trainer assignments; Rival/Friend grouped consistency; no P1 promotion |
...
| P1 | Rival Carries Starter Through Game | targeted visual smoke passed | `FVX-FOE-012` | Player Charmander -> Rival Squirtle lokal bestaetigt, aber kein Full-Playthrough und keine all-starter-choice matrix | breiteres Sampling nur bei Bedarf; keine P1-Promotion | 179, 192, 201, 207, PR117 |
...
| P1 | Trainer Names/Class Names / Sprite Sync | Trainer Names passed; Class Names textlabel-only without Sync; Sprite Sync targeted-smoke passed | `FVX-FOE-013` | `MODE-TRAINER-CLASS-SPRITE-SYNC` targeted visual-smoke-confirmed, aber kein Full-Playthrough und keine P1-Promotion; ohne Sync bleibt legacy/textlabel-only | nur bei Regression oder breiterem Sampling erneut vertiefen; keine P1-Promotion | 190, 206, PR116 |
...
| 2 | Stable Visual Profile + Starter Pokemon + Intro Mon | groesstes aktuelles Normalprofil laenger samplen | Stable Visual Profile passed, Oak-Lab Rival Sync passed, PR #117 combined visual smoke passed und Intro Mon visual fix targeted-smoke passed | lokaler Smoke nur bei Bedarf mit Starters ON, Intro Mon ON, Trainer Class Names nach Scope, Special-Wild OFF |
| 3 | Rival Carries Starter Through Game | weitere Starter-/Rival-Auftritte samplen | ein Counter-Pfad ist targeted-smoke-bestaetigt, aber keine all-starter-choice matrix und kein Full-Playthrough | breiteres lokales Sampling nur bei Bedarf |
