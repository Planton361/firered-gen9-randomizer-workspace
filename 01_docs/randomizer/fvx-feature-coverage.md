...
- Evidence: save/log/output/reload successful, `starterHeldItemReloadMismatches=0`, no bad/TM/invalid/unloaded/fallback/placeholder writes, and Wild/Trainer/Field/Pickup/Shop isolation preserved.
- Starter Ban Bad remains unpromoted.
...
- Starter Held Items remain unpromoted.

# 2026-05-15 - FVX Held Items coverage update

- Regular Trainer Held Items are covered as GUI-compatible in the tested CFRU/DPE Gen9-BPRE no-filter scope after Diagnose 143.
- Evidence: save/log/output/reload successful, `regularTrainerHeldItemReloadMismatches=0`, Boss/Important/shouldNotGetBuffs preserve counters `0`, invalid/unloaded/fallback/placeholder writes `0`, and Wild/Starter/Field/Pickup/Shop scopes unchanged.
- Trainer Held Items class coverage is now complete for Boss/Important/Regular no-filter scopes; Trainer filters and Starter Held Items remain unpromoted.
...
- Wild/Encounter Held Items + Ban Bad are covered as GUI-compatible in the tested CFRU/DPE Gen9-BPRE scope after Diagnose 140.
- Evidence: save/log/output/reload successful, `wildHeldItemReloadMismatches=0`, `badWildHeldItemWrites=0`, bad held items `174/0/0`, and Trainer/Starter/Field/Pickup/Shop scopes unchanged.
- Trainer Held Items and Starter Held Items remain unpromoted.
...
- `opShopItemPoolCandidates=9`, `opShopItemPoolExcluded=9`, `opShopItemBannedWrites=0`.
- Der Gesamt-Scope Shop Item Bans bleibt nur fuer Ban-Kombinationen offen; `FVX-ITEM-008` und `FVX-ITEM-009` sind nicht hochgestuft.
...
- Evidence: save/log/output/reload successful, `shopItemReloadMismatches=0`, shop counts/lengths stable, skipped shops preserved, prices untouched and Field/Pickup/Held scopes unchanged.
- `FVX-ITEM-006..009` remain `Write modelliert`; Shop Random, Shop Bans, Guarantee Evolution/X Items and Balance Prices / Cheap Rare Candies need separate smokes.
- Field Items, Pickup and Held Items are not affected.
...
- `FVX-ITEM-010 Pickup Ban Bad`: planned next as a separate Pickup-only smoke with `banBadRandomPickupItems=true`; not yet upgraded by Diagnose 119.
- Field Items, Shops and Held Items are not affected by this Pickup Ban Bad plan.
...
- Diagnose 117 identifies the next likely fix target as the Pickup table locator, not the random item pool or direct item write.
- A later fix must make Pickup table localization reload-stable after item IDs change.
- Pickup Ban Bad remains blocked until Pickup Random without Ban Bad is reload-stable.
- Field Items remain GUI-compatible only in their tested Field-Items scopes; Shops, Pickup Ban Bad and Held Items remain separate.
