...
Recommended baseline: `VAR_TRAINER_AI_MODE = 1`, equivalent to "add `AI_SCRIPT_SEMI_SMART` to regular trainer AI where `AI_SCRIPT_CHECK_GOOD_MOVE` is absent." Use `FLAG_SMART_TRAINER_AI` only if the project intentionally wants a single non-tiered toggle. Keep wild AI separate behind a new wild-specific control.
