...
- Treat `08_tests/randomizer/cfru-name-rater-centers-rollout.md` as updated to `COMPLETE_ROLLOUT_PENDING_MANUAL_SMOKE`.
- Manual smoke must run after a clean CFRU build with `python3 scripts/clean.py BUILD` and `python3 scripts/make.py`, then every implemented Pokecenter must be entered from outside.
- Smoke the prior Viridian/safe-Kanto entries plus the newly added high-count Kanto, Indigo Plateau and Sevii maps for map load, added Name Rater visibility/interactability, Nurse, PC, warps, existing NPCs, and Name Rater No/Cancel/Confirm paths.
- Give extra attention to high-count Kanto maps, Indigo Plateau layout, Four Island BG signs, and One Island Bill/Celio/Network Machine plus leave-island coord triggers.
- Do not promote beyond pending-smoke status until the manual rollout smoke is recorded. Continue excluding Faster Intro, Oak/Lab/Parcel, Bill-Sevii, Repel-Reuse, Hidden Items, itemball graphics, Field Items, Randomizer writer, UPR-FVX/DPE changes, ROMs, saves, builds, tool binaries, screenshots, raw logs, private paths, tokens, secrets and `.env` data from commits.
