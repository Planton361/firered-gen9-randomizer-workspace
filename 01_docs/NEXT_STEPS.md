...
CFRU/DPE Move Names / Descriptions Text/Menu-Scope ist read-only geplant. Diagnose: `08_tests/randomizer/088_move_names_text_menu_scope_plan.md`.

`FVX-MOVE-001/002/003/004/006` sind GUI-kompatibel. `FVX-MOVE-005` bleibt getrennt vom MoveData-Byte-Writer-Scope.

Planentscheidung aus 088: ein enger Name-only Reload-Smoke ist realistisch; Move Descriptions / Text/Menu-Repointing bleibt vorerst zurueckgestellt.

Naechster aktiver Arbeitsblock: sanitiserter Name-only Reload-Smoke fuer `FVX-MOVE-005` ohne Description-, Pointer-, Repointing- oder Text/Menu-Umsetzung.

## Priorisierte naechste Arbeitsbloecke

1. Move Names fixed-length Reload-Smoke
...
2. Move Names / Descriptions Text/Menu-Scope Plan halten
...
3. MoveData Fairy-Type-Byte-Fix post-merge halten
...
4. MoveData Types Reload-Smoke historisch einordnen
...
5. MoveData Power/Accuracy/PP Reload-Smoke reviewen
...
6. PRs fuer Wild-Filter-Carrier-Nullslot-Fix reviewen
...
7. PRs fuer Trainer Type Diversity Null-Type Fix reviewen
...
8. PRs fuer Evolution Same Typing Null-Type Fix reviewen
...
9. PR fuer `FVX-TRAIT-018` Similar Strength Normalized Reload reviewen
...
10. Wild-Suboptionen konservativ halten
...
11. Spaetere TypeEffectiveness-Kombinationen nur bei Bedarf
   - Nicht mit MoveData, Palette, Items, Graphics, Text/Menu oder Species-Type-Write vermischen.

12. `compat/upr-fvx-cfru-dpe-palette-randomization-preserve-repoint`
...
13. `compat/upr-fvx-cfru-dpe-field-items-shops-pickup-scope-and-write`
