...
DEFAULT_FIXTURES = Path(__file__).resolve().parent / "fixtures" / "synthetic_fixtures.json"
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if args.command == "validate":
            emit({"fixture_count": len(fixtures), "schema_version": fixtures[0]["schema_version"],
                  "status": "PASS"})
