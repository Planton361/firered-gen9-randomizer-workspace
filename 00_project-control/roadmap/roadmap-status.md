# Roadmap status update - CFRU 29-slot TM/HM itemball candidate pinned

- Status impact: `ROLLOUT_PENDING_RANDOMIZER_AND_RUNTIME_SMOKE`. CFRU candidate
  `08b869032735118539411adbcffa421c8a697caa` is pinned for review; Draft PR
  #35 remains open and must not be merged before acceptance evidence exists.
- The exact 29-slot policy remains 28 TMs plus HM07 at existing gold graphics
  ID `0x065C` (source `0x005C`, table 6, low byte 92, palette `0x1106`);
  NatDex ID 67 stays reference-only. HM07's item, script, flag, pickup and
  preserve-only policy are unchanged.
- Static overlay, generator, build and insertion gates pass. New runtime
  spotchecks and the six-row fresh UPR-FVX matrix remain the required separate
  acceptance gates; no graphics or palette work is needed.
...
- Added sanitized analysis and smoke docs for the dry-diff result.
- Status impact: the project now has a reviewable Base Stats-only dry-diff path that honors the reviewed alias table, skips Species open-risk and reviewed ignores, and keeps Ability blockers out of safe candidate promotion.
- Dry-diff result: `PASS_READ_ONLY_WITH_BLOCKERS`. Counts: `1317` tested Species, `29` Species `open-risk` skipped, `167` reviewed Species ignores skipped, `65` Ability-blocked Species skipped from safe candidate promotion, `4` missing local entries after alias/ignore handling, and `225` safe candidate Species with non-Ability field diffs.
- Showdown `pokedex.ts` did not provide Catch Rate, EXP Yield, EV Yield, or Growth Rate in this pass; those fields remain blocked pending a secondary source.
...
- Expanded the reviewed Pokemon Showdown-to-CFRU/DPE alias table in `07_scripts/data_audit/showdown_aliases.json`.
- Status impact: the move-split batch covers explicit reviewed Z/Max/GMax physical-special Move split aliases without changing CFRU/DPE data tables.
- Alias table now contains 169 entries: Species `form-name` 4, Species `gmax-giga` 32, Species `local-shortform` 55, Move `split-move` 69, Move `hidden-power-variant` ignore 1, Move `spelling` 1, Ability `name-mismatch` 1, and Ability `behavior-risk` 6.
- External-data smoke summary: Move reviewed classifications increased to 86 Showdown-only keys and 139 local-only keys; 18 Showdown-only Move keys and 4 local-only Move keys remain uncategorized.
- Species and Ability unresolved buckets were not expanded in this batch; no broad regex rule was added to hide uncertain mappings.
- No CFRU/DPE Pokemon data table, UPR-FVX code, submodule pin, Pokemon Showdown data copy, ROM path, ROM hash, CRC, full log, screenshot, save, emulator state, output ROM, build artifact, tool binary, private path or P1 promotion is added.

# Roadmap status update - Pokemon data alias table batch 2

- Expanded the reviewed Pokemon Showdown-to-CFRU/DPE alias table in `07_scripts/data_audit/showdown_aliases.json`.
- Status impact: Batch 2 covers reviewed Species-only name-shape mappings for regional/local shortforms and GMax/Giga forms without changing CFRU/DPE data tables.
- Alias table now contains 107 entries: Species `form-name` 4, Species `gmax-giga` 32, Species `local-shortform` 55, Move `split-move` 7, Move `hidden-power-variant` ignore 1, Move `spelling` 1, Ability `name-mismatch` 1, and Ability `behavior-risk` 6.
- External-data smoke summary: Species reviewed classifications increased to 91 Showdown-only keys and 95 local-only keys; 228 Showdown-only Species keys and 126 local-only Species keys remain uncategorized.
- Move and Ability unresolved buckets remain visible for later batches; no broad regex rule was added to hide uncertain mappings.
- No CFRU/DPE Pokemon data table, UPR-FVX code, submodule pin, Pokemon Showdown data copy, ROM path, ROM hash, CRC, full log, screenshot, save, emulator state, output ROM, build artifact, tool binary, private path or P1 promotion is added.

# Roadmap status update - Pokemon data reviewed alias table

- Added the first reviewed Pokemon Showdown-to-CFRU/DPE alias/ignore table in `07_scripts/data_audit/showdown_aliases.json`.
- Updated `07_scripts/data_audit/showdown_mapping_audit.py` so the read-only mapping audit loads the alias table and separates reviewed classifications from still-uncategorized keys.
- Added review handoff `01_docs/analysis/pokemon-data-reviewed-alias-table.md` and sanitized smoke `08_tests/randomizer/pokemon-data-reviewed-alias-table.md`.
- Status impact: name-shape differences can now be classified in a machine-readable way before any Pokemon data-table update. No support level is promoted.
- Initial coverage: Ogerpon Terastal forms, GMax/Giga Species examples, regional/local shortforms, Z/Max/GMax physical-special Move splits, Hidden Power typed-variant ignore rule, `visegrip` / `vicegrip`, and Ability aliases marked as behavior risks.
- External-data smoke summary: the table classified 12 Showdown-only Species keys, 16 local-only Species keys, 24 Showdown-only Move keys, 15 local-only Move keys, and one Ability name mismatch on each side; the remaining unresolved mappings stay uncategorized.
- No CFRU/DPE Pokemon data table, UPR-FVX code, submodule pin, Pokemon Showdown data copy, ROM path, ROM hash, CRC, full log, screenshot, save, emulator state, output ROM, build artifact, tool binary, private path or P1 promotion is added.
...
- Status impact: Wild Encounter Base-vs-Output Audit is available as diagnostic-only tooling. It makes no writer/randomizer behavior change and does not promote Wild Pokemon to P1.
- Scope: modeled Gen3 base `WildPokemon` table path, with per-slot Base-vs-Output comparison and total/changed/unchanged/changed percentage summary.
- CFRU/DPE special/runtime wild sources remain follow-up if ingame behavior and the modeled-table audit diverge.
- No ROM paths, hashes, CRCs, full logs, screenshots, saves, emulator states, output ROMs, workspace-side UPR-FVX/CFRU/DPE code changes or P1 promotion are added.

# Roadmap status update - Combined trainer visual runtime smoke
...
- PR #116 finalizes the opt-in `MODE-TRAINER-CLASS-SPRITE-SYNC` behavior after the per-trainer assignment and Rival/Friend grouped-consistency follow-ups.
- Semantics: `Randomize Trainer Names` remains personal-name-only; `Randomize Trainer Class Names` remains legacy/textlabel-only without Sprite Sync; with Sprite Sync enabled, class label, `trainerClass` and visible `trainerPic` follow the class assignment.
- Status impact: Trainer Class Sprite Sync is locally smoke-confirmed for the targeted visual path. Regular trainers use per-trainer class/sprite assignments; Rival/Friend rows keep grouped class/sprite consistency across appearances; runtime-source rows are included where eligible.
- Sanitized local evidence confirms Viridian Forest Bug Catcher classes randomize per trainer, Rival keeps the first randomized sprite across later appearances, other sampled trainers appeared aligned, and no garbled sprite or crash was reported.
- Caveat: targeted visual smoke only, not a full playthrough, broad route/category sweep or P1 promotion.
- No ROM paths, hashes, CRCs, full logs, screenshots, saves, emulator states, output ROMs, workspace-side UPR-FVX/CFRU/DPE code changes or P1 promotion are added.
...
- PR #109 fixes the CFRU/DPE Gen9 BPRE Intro Mon visual mismatch by syncing the Nidoran female normal front-image and normal-palette pointer-table entries to the selected intro species' asset pointers.
- Added sanitized evidence file `08_tests/randomizer/205_intro_mon_visual_source_fix_smoke.md`.
- Local smoke confirms the visible Oak intro sprite changed away from Nidoran female after the fix, with no crash, freeze or garbled sprite observed.
- Status impact: `FVX-GEN-003` / Intro Mon visual mismatch is locally fixed for the targeted smoke.
- Caveat: targeted ingame smoke only, not full-playthrough coverage or a P1 promotion.
- No ROM paths, hashes, CRCs, full logs, screenshots, saves, emulator states, output ROMs, workspace-side UPR-FVX/CFRU/DPE code changes or P1 promotion are added.

# Roadmap status update - Intro Mon visual source diagnostics sync

- Synced merged UPR-FVX PR #107 into the workspace pin at `a7e098a5158d824b1ddec62a286f2a6ffafce8e4`.
...
- The status model now records the setting semantics: `No Random Intro Mon` is the negative GUI option, while `randomizeIntroMon=true` is the active Intro Mon randomization path.
- `MODE-INTRO-RANDOM` sets true; `MODE-NO-RANDOM-INTRO` and `FVX-GEN-003` set false.
- This is diagnosis-only and does not add a visible Intro Mon fix, writer change, ROM execution by Codex or P1 promotion.
- Next local evidence should be sanitized candidate-source output plus observed visible Intro Mon label, without ROM paths, hashes, full logs, screenshots, saves, emulator states or output ROM details.
...
- PR #105 makes generic `RUNTIME-SOURCE` trainers randomizer-eligible as regular trainers while preserving known Rival 2/Brock special tags.
- Added sanitized evidence file `08_tests/randomizer/203_runtime_source_trainer_randomization_smoke.md`.
- Trainer/Foe runtime-source strict sync plus randomizer eligibility is locally confirmed for Viridian Forest trainer IDs `531/532`: targeted audit loaded/raw parties match and the observed battle now shows randomized Eiscue instead of vanilla Metapod/Caterpie.
- Loaded-mismatch, invalid-pointer, empty-party and out-of-range rows remain follow-up scope.
- No ROM paths, hashes, CRCs, full logs, screenshots, saves, emulator states, output ROMs, workspace-side UPR-FVX/CFRU/DPE code changes or P1 promotion are added.
...
# Roadmap status update - Exact coverage batches 03-18

- Recorded sanitized exact-coverage Batch 03 through 18 CLI log-smoke/helper results.
- `08_tests/randomizer/201_exact_coverage_batches_03_18.md` documents 165 generator-capable exact/cumulative/mode profiles processed across Batches 03 through 17 with all PASS profiles at 0 bad markers and 0 warnings, plus 4 expected-fail Gen-Limit `MODE-*` helper validations in Batch 18.
- Updated `fvx_feature_test_status_matrix.tsv` for affected generator-capable rows across TM/Tutor, Wild, Foe, General/Traits, Starters/Statics/Trades, Moves, Graphics/Palettes, Misc, Types, cumulative coverage and exact Foe/Wild/Type/Intro mode overlays.
- Caveated rows remain caveated where appropriate: Graphics/Palettes, sensible Trainer Held Items, Intro Mon visual confirmation, Gen-Limit unsupported Settings format, Special-Wild and static placeholders. Manual/unsupported and out-of-scope rows are not promoted.
- Progress dashboard snapshot, package summaries and completed-diagnosis references now mention Batches 03-18 without shortening the full Feature-ID list.
- No ROM paths, hashes, CRCs, full logs, screenshots, saves, emulator states, output ROMs, UPR-FVX/CFRU/DPE code changes or P1 promotion are added.
...
- `08_tests/randomizer/199_exact_coverage_batch_01.md` documents 19 profiles processed with dry-run disabled, all PASS, 0 bad markers and 0 warnings.
...
- Batch 01 converts the targeted exact-coverage gaps to `PASS_LOG` evidence while retaining ingame follow-up requirements and existing stronger statuses where applicable.
- Progress dashboard snapshot and completed-diagnosis references now mention Batch 01 without shortening the full Feature-ID list.
- No ROM paths, hashes, CRCs, full logs, screenshots, saves, emulator states, output ROMs, UPR-FVX/CFRU/DPE code changes or P1 promotion are added.
...
- Evolution row stride is fixed for the CFRU/DPE Gen9 path.
- CFRU/DPE uses `EVOS_PER_MON=16`; UPR-FVX now uses `evolutionSlotsPerSpecies=16` and `evolutionRowSize=0x80` for CFRU/DPE evolution read/write/report.
- Root cause: the previous UPR-FVX path used vanilla 5-slot rows (`0x28` bytes), which made the report read the private input ROM incorrectly and could corrupt output evolutions.
- Sanitized local report evidence after PR #82 shows Input ROM starter chains correct and new Output ROM starter chains correct.
...
- Next recommended option block: a separate Trainer Names/Class Names or first Items/Moves/Abilities slice, with Special-Wild systems still disabled.
- No ROM paths, hashes, CRCs, full logs, screenshots, saves, emulator states, output ROMs, UPR-FVX/CFRU/DPE code changes or P1 promotion are added.

# Roadmap status update - GUI-4B no-swarms pass

- UPR-FVX PR #79 and PR #80 are synced into the workspace pin at `226bcacc4f66cee5689caa128d5e35ef4acc001d`.
- CFRU PR #5 is synced into the workspace pin at `c4c90373fe7f24acd5dcfa3a8fbdd5cb573bfe29`.
- GUI-4B Wild Standard/Fallback plus Trainer Pokemon core plus Pokemon Movesets -> Random completely passed in sanitized local evidence.
...
- Learnset empty-moveset crash is resolved for this scope; the `SpeciesMovesetRandomizer` `IndexOutOfBoundsException` was not reproduced.
- Swarm-Frigibax is neutralized for normal randomized walkthroughs by CFRU `SWARM_CHANCE=0`; Route 1 no-swarm rebuild did not observe Swarm-Frigibax and an example Route 1 encounter was Urshifu Lv3 displayed correctly.
...
- UPR-FVX PR #46 is merged and workspace-pinned at `c86221d758bece06b216b1f4fb23dd8e4a6c8ec0`.
- The pinned `WildCatchLevelDecisionTest` covers Set Minimum Catch Rate, Catch Em All Mode and Balance Low Level Encounters + Level Modifier with synthetic Non-ROM data.
- Statuswirkung: `FVX-WILD-007`, `FVX-WILD-010` and `FVX-WILD-012` are now `tested-non-rom`, not P1-supported; no ROM-/Reload-Evidenz, ROM-Smoke, output-ROM or Randomizer run was executed.
...
- `FVX-TRAIT-026` remains a helper flag for `024/025`, not a standalone promotion.
- No ROM-Smoke, Randomizer run, build, code change or submodule change was performed.
...
- Result: PASS with save/log/output/reload successful, `importantTrainerHeldItemReloadMismatches=0`, preserve counters for Boss/Regular/shouldNotGetBuffs all `0`, and no invalid/unloaded/fallback/placeholder writes.
- Regular Trainer Held Items are the next separate scope.

# 2026-05-15 - Boss Trainer Held Items smoke update

- Diagnose 141 records Boss Trainer Held Items Write/Reload-Smoke.
- Result: PASS with save/log/output/reload successful, `bossTrainerHeldItemReloadMismatches=0`, preserve counters for Important/Regular/shouldNotGetBuffs all `0`, and no invalid/unloaded/fallback/placeholder writes.
- Important Trainer Held Items are the next separate scope.

# 2026-05-15 - Wild Held Items Ban Bad smoke update

- Diagnose 140 records Wild/Encounter Held Items + Ban Bad Write/Reload-Smoke.
- Result: PASS with save/log/output/reload successful, `wildHeldItemReloadMismatches=0`, `badWildHeldItemWrites=0`, and no invalid/unloaded/fallback/placeholder writes.
- Wild/Encounter Held Items are covered with and without Ban Bad; Trainer Held Items are the next separate scope.
...
- `opShopItemBannedWrites=0`; Reload, Skip-Shops, Preis- und Fremdscopes bleiben stabil.
- Ban Bad und Ban Regular sind bereits aus Diagnose 127/128 einzeln belegt; Ban-Kombinationen bleiben separat.
