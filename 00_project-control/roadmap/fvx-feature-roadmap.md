...
| Graphics | 6 | Gemischt | Diagnose 093 findet nur normal-palette-only single-owner/decompressible Kandidaten; Shiny/shared/invalid/missing/decode-failed bleiben preserve-only, Custom Player Graphics separat modellieren |
...
| P1.3c | `analysis/upr-fvx-cfru-dpe-palette-single-owner-normal-only-fix-scope-plan` | Graphics | naechster Schritt: normal-palette-only, single-owner/decompressible Fix-/Smoke-Scope planen; Shiny/shared/invalid/missing/decode-failed preserve-only |
| P1.3d | `compat/upr-fvx-cfru-dpe-palette-randomization-preserve-repoint` | Graphics | spaeterer Fix/Smoke nur nach Scope-Plan: entweder normal-only Single-owner Proof oder vollstaendige Secondary-Pointer-/Shared-Pointer-Policy |
